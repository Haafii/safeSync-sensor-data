...
@app.get("/latest-sensor-data/", response_model=SensorDataResponse)
async def get_latest_sensor_data():
    latest_data = collection.find().sort('_id', -1).limit(1)
    latest_data_item = next(latest_data, None)  # Get the latest item (if exists)
    
    if latest_data_item:
        return SensorDataResponse(**convert_objectid_to_str(latest_data_item))
    else:
